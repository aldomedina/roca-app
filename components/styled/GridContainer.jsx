...
export const Wrapper = styled.div`    
    ...
    box-shadow: ${({theme, border, borderWidth=2}) => border ? `0 0 0 ${borderWidth}px inset ${theme.colors[border]}`  : 'none'};   
`
...
export const GridContainer = styled.div`
    ...
    ${props => !!props.bgImg && `
        ...
        `};            
    width: 90vw;   
    min-height: ${({theme, fullH}) => !!fullH && `calc(${100 * theme.vh}px - 10vw)`}; 
    display: grid;
...
export const GridChild = styled.div`
    ...
    background-color: ${({theme, bgColor}) => bgColor ? theme.colors[bgColor] : 'transparent'};
    border: ${({theme, border, borderWidth=2}) => border ? `${borderWidth}px solid ${theme.colors[border]}`  : 'none'};    
